...
int compareStatesDFS(State S1, State S2) {
    return (S1.Demon == S2.Demon && S1.Monk == S2.Monk && S1.Boat_Loc == S2.Boat_Loc);
}

int findStateDFS(State state, Stack openStack) {
    while (!emptyStack(openStack)) {
        if (compareStatesDFS(top(openStack)->state, state))
            return 1;
        pop(&openStack);
    }
    return 0;
...
Node *DFS_Algorithm(State state) {
    ...
    while (!emptyStack(OpenDFS)) {
        ...
        for (opt = 1; opt <= 5; opt++) {
            ...
            makeNULLState(&newState);
            if (call_operator(node->state, &newState, opt)) {
                if (findStateDFS(newState, CloseDFS) || findStateDFS(newState, OpenDFS))
                    continue;
                Node *newNode = (Node *)malloc(sizeof(Node));
                newNode->state = newState;
                newNode->parent = node;
